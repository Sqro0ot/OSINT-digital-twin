...
const FocusOnSelected: React.FC<{
  devices: Device[];
  selectedAssetId: number | null;
  ...
  useEffect(() => {
    ...
    if (!isValidCoord(dev.lat) || !isValidCoord(dev.lon)) {
      return;
    }
...
const MapView: React.FC<MapViewProps> = ({
  ...
  const center: [number, number] = [43.2389, 76.8897]; // Алматы
  console.log('DEVICES', devices);
  const visibleDevices = devices.filter(
    (d) => isValidCoord(d.lat) && isValidCoord(d.lon),
  );
        ...
        >
          ...
          {visibleDevices.map((device) => (
            <Marker
              ...
              icon={getMarkerIcon(
                device.risk_level,
                device.id === selectedAssetId,
              )}
            >
              <Popup>
                <div style={{ maxWidth: 260 }}>
                  <div>
                    <b>{device.name || `Camera ${device.id}`}</b>
                  </div>

                  {device.cvss_max != null && (
                    <div>CVSS max: {device.cvss_max.toFixed(1)}</div>
                  )}
                  ...
                  {device.last_seen && (
                    <div>Last seen: {device.last_seen}</div>
                  )}
            ...
            >
              ...
              {newCves.length > 0 && (
                <div style={{ marginTop: 4, color: '#8c8c8c' }}>
                  New CVEs: {newCves.slice(0, 3).join(', ')}
                  {newCves.length > 3 && ' ...'}
                </div>
              )}
            </div>
